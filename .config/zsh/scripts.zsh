...
panpdf(){
    pandoc --pdf-engine=xelatex --highlight-style=zenburn \
        --include-in-header=/Users/sleep/.pandoc/head.tex -V fontsize=10pt -V colorlinks --toc \
        -V linkcolor=red -V urlcolor=teal -V filecolor=magenta -s \
        -f markdown+tex_math_dollars+grid_tables --mathjax --columns=10\
        $1 -o ${1%%.*}.pdf && open ${1%%.*}.pdf
}
